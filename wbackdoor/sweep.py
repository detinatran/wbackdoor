"""
Sweep theta_max x rho and produce the localization/leak tradeoff figures.

For each (theta_max, rho) it trains a victim and records, at the reference dose:
  - displacement   (target-limb manipulation magnitude)   [mm]
  - nontarget_mpjpe (leakage into the rest of the body)    [mm]
  - clean_mpjpe                                            [mm]
  - dose_response: spearman, ramp_minus_step (analog necessity)
  - asr frac_moved

Outputs:
  sweep_results.json / .csv
  fig_tradeoff.png        leak vs displacement, one line per rho across theta
  fig_heatmaps.png        rho x theta heatmaps (leak, displacement, ramp-minus-step)

COST WARNING: trains len(theta)*len(rho) models. Use --epochs to shorten for the
sweep (e.g. 100) and a coarse grid first. Run on the machine with the real dataset.

Usage:
  python sweep.py --config configs/attack.yaml \
      --theta 20 30 40 50 60 --rho 0.01 0.02 0.05 0.1 --epochs 100
"""
