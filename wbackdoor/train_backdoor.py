"""
Train a victim HPE model on poisoned Person-in-WiFi-3D data and evaluate the
analog (dose-response) backdoor.

Usage:
    python train_backdoor.py --config configs/attack.yaml
Point `dataset_root` at the real data/person_in_wifi_3d to run for real.
"""
...
def build_trigger(cfg):
    """trigger_type: microdoppler (mặc định) | trojan.
    trojan đọc pattern m đã tối ưu sẵn từ cfg['trojan_npy'] (chạy run_trojan.py trước)."""
    ttype = cfg.get('trigger_type', 'microdoppler').lower()
    if ttype == 'trojan':
        from attack.trojan import TrojanTrigger
        m = np.load(cfg['trojan_npy'])
        return TrojanTrigger(m)
    if ttype == 'wanet':
        from attack.wanet import WaNetTrigger
        return WaNetTrigger(k=cfg.get('wanet_k', 4), seed=cfg.get('seed', 0))
    if ttype == 'wanet_cplx':
        from attack.wanet_cplx import WaNetComplexTrigger
        return WaNetComplexTrigger(k=cfg.get('wanet_k', 4), seed=cfg.get('seed', 0))
    if ttype == 'ftrojan':
        from attack.ftrojan import FTrojanTrigger
        return FTrojanTrigger(freq_bins=tuple(cfg.get('freq_bins', (2, 5, 8))),
                              mag=cfg.get('ftrojan_mag', 6.0), seed=cfg.get('seed', 0))
...
def main():
    ...
    ap.add_argument('--save', default=None, help='luu model state_dict ra file .pt (surrogate cho Trojan)')
    a = ap.parse_args()
    ...
    if a.save:
        torch.save({'state_dict': model.state_dict(),
                    'cfg': {k: cfg[k] for k in ('model', 'experiment_name') if k in cfg},
                    'subcarrier_num': 180}, a.save)
        print(f'\n[saved surrogate] {a.save}')
